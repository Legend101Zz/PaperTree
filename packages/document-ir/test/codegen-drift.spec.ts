...
const GENERATED_DIRS = [
  "src/generated",
  "python/papertree_document_ir/generated",
] as const;
...
describe("codegen drift", () => {
  ...
  it("every generated object closes its field set", () => {
    ...
    for (const name of [
      ...
      const models = classes.filter((c) => !/(StrEnum|BaseModel)/.test(text.slice(text.indexOf(c), text.indexOf(c) + 120)));
  ...
  it("no bare \\s reaches a non-JavaScript regex engine", () => {
    ...
    for (const name of [
      ...
      for (const line of text.split("\n")) {
        ...
        expect(/\\s/.test(line), `${name}: a raw \\s survives into a Python regex:\n  ${line}`).toBe(
          false,
        );
      }
    }

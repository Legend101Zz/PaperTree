...
const ajvMessage = (c: Case): string => ajv.errorsText(validators[c.schema].errors, { separator: "; " });
...
describe("the known divergences are exactly the documented ones", () => {
  ...
  it("no divergence class exists that DESIGN.md §12 does not name", () => {
    const classes = [...new Set(annotated.map((c) => (c.divergence as Divergence).class))].toSorted();
...
describe("the named acceptance criteria, restated against the generated bindings", () => {
  ...
  it("an unknown block type validates under Zod, not merely under ajv", () => {
    for (const name of ["unknown-block-type", "unknown-type-carries-payload", "unknown-relation-type"]) {
  ...
  it("LLM-authored text in a source field fails under Zod", () => {
    for (const name of ["block-source-llm", "block-source-model", "block-source-vlm", "block-generated-by"]) {
  ...
  it("an unpaired surrogate is rejected by Zod in constrained AND unconstrained strings", () => {
    for (const name of ["lone-surrogate-constrained-string", "lone-surrogate-unconstrained-string"]) {

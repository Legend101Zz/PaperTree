...
function diverging(
  ...
  cases.push({ name, schema: "paperir", expect: ajv, reason, origin: ATTACK, document, divergence });
}
...
addDerivation(
  ...
  "valid",
  "a Derivation of an unknown kind validates",
  { ...DERIVATION, kind: "brand_new_kind_v9" },
);
add("ir-version-1-4-0", "valid", "any 1.x validates (D10)", paper((p) => (p.ir_version = "1.4.0")));
add("ir-version-2-0-0", "invalid", "a 2.x document must not validate (D10)", paper((p) => (p.ir_version = "2.0.0")));
add(
...
for (const r of [45, -90, 360]) {
  add(`page-rotation-${r}`, "invalid", "rotation is one of 0/90/180/270", paper((p) => (p.pages[0].rotation = r)));
}
for (const [label, u] of [["zero", 0], ["negative", -1], ["denormal", 1e-9]] as const) {
  add(`page-user-unit-${label}`, "invalid", "user_unit is bounded to [0.001, 1000]", paper((p) => (p.pages[0].user_unit = u)));
}
for (const [label, w] of [["zero", 0], ["negative", -612], ["huge", 1e6]] as const) {
  add(`page-width-${label}`, "invalid", "page width is bounded > 0 and <= 20000", paper((p) => (p.pages[0].width = w)));
}
add(
...
add("repair-vlm-applied", "invalid", "a vlm_substitution may not be applied (D4)", setRepair(modelRepair({ applied: true })));
add(
  "repair-ocr-applied",
  "invalid",
...
add("repair-model-proposed", "valid", "an unapplied, fully attributed model repair is accepted", setRepair(modelRepair()));
add(
  "repair-deterministic-with-model-id",
  "invalid",
  "a DETERMINISTIC repair may not name a model (D4, review finding: FATAL)",
  setRepair({ kind: "dehyphenate", applied: true, at: 49, from: "resid-\nual", to: "residual", model_id: "gpt-4o" }),
);
add(
...
add("repair-missing-applied", "invalid", "`applied` is required", setRepair({ kind: "ligature", from: "a", to: "b" }));
add(
  "repair-unknown-kind",
  "invalid",
...
add(
  ...
  setAlts([
    { parser: "vlm-repair", authored_by: "model", text: "<the model's reading>", confidence: 0.88, decision: "not_selected" },
  ]),
);
add(
...
add("generation-zero", "invalid", "generations are 1-based (D13)", paper((p) => (p.generation = 0)));
add("generation-seven", "valid", "generation 7 is a legitimate re-parse", paper((p) => (p.generation = 7)));

// --- no field a real parser must fabricate -------------------------------------------------------------
for (const s of ["pending", "parsing"]) {
  add(`status-${s}`, "invalid", "pending/parsing are job states, not document states (D15)", paper((p) => (p.status = s)));
}
for (const s of ["partial", "complete", "failed"]) {
  add(`status-${s}`, "valid", "the three document states", paper((p) => (p.status = s)));
}
add(
...
addDerivation("derivation-empty-derived-from", "invalid", "a Derivation must point at at least one source block", {
  ...DERIVATION,
  derived_from: [],
});
...
addDerivation("derivation-extra-field", "invalid", "a Derivation rejects unknown fields, like everything else", {
  ...DERIVATION,
  rendered_as: "source",
});
addDerivation(
...
add(
  ...
  paper((p) => {
    p.relations.push({ type: "9lives", from: ID.para, to: ID.equation, confidence: 0.5, provenance: "x" });
  }),
  F03,
);
add(
    ...
    plainBlock({
      ...
      payload: {
        ...
        grid: {
          ...
          cols: 2,
          cells: [
            { cell_id: "blk_c2c3c4c5c6c7c2c3", r: 0, c: 0, polygon: [[1, 1], [2, 1], [2, 2]], text: "a", is_header: true },
            { cell_id: "blk_c3c4c5c6c7c2c3c4", r: 0, c: 1, polygon: [[2, 1], [3, 1], [3, 2]], text: "b" },
          ],
        },
      },
    }),
...
add(
  ...
  withExtraBlock(
    plainBlock({ type: "table", block_id: "blk_t2t3t4t5t6t7t2t3", payload: { display: true, image: null } }),
  ),
  F03,
);
add(
...
add(
  ...
  withExtraBlock(
    plainBlock({
      ...
      block_id: "blk_t2t3t4t5t6t7t2t3",
      payload: {
        grid: { rows: 1, cols: 1, cells: [{ cell_id: "blk_c2c3c4c5c6c7c2c3", r: 0, c: 0, polygon: [[1, 1], [2, 1], [2, 2]], generated_by: "gpt-4" }] },
      },
    }),
  ),
  F03,
...
attack(
  ...
  payloadBlock(
    ownProto({}, { derivation_id: DERIVATION.derivation_id, derived_from: [ID.para] }),
  ),
);
attack(
...
attack("uri-whitespace-tab", "invalid", "a tab is whitespace to every engine", withUri('"r2://a\\tb.webp"'));
attack(
  "uri-whitespace-none",
  "valid",

...
| `worker/perf.spec` | **PARTIAL** | time ✅ — re-derived 2026-08-03 by `uv run python -m papertree_evaluation speed` (1 warm-up pass discarded per parser per paper, 3 counted trials, all 8 papers / 195 pages): **169 ms/page** corpus-amortised median, min 168 – max 171. Slowest paper `neural-odes-mathheavy` **337 ms/page** median (min 334 – max 346) — and over 8 papers `operational()`'s p95 *is* the maximum (`int(8 × 0.95) == 7`), so that is the p95 the bar names. Both against the 1500 ms bar, on a machine **not declared quiesced** (load average 5.04 at start), which can only inflate them. Replaces `p50 305 / p95 568`, which came from one un-warmed run (#53). **Memory ❌ NOT MET — the bar is straddled, on 29 runs.** `gpt3-longform-singlecol` (75 pp, fresh subprocess each time), re-derived 2026-08-03 on `main` at `0e63488` in five batches at different machine loads: quiesced (n=7) **496.3–497.0**; 20 busy loops (n=7) **406.0–496.9**; 20 loops + the suite (n=5) **497.2–499.3**; load 5.2 (n=5) **440.5–498.6**; load 3.9 (n=5) **496.6–500.4**. The distribution is **bimodal** — 23 of 29 in a tight upper mode spanning 495.4–500.4, and all 5 readings below 460 come from a loaded box, so load drops the high-water mark rather than scattering it. **1 of 29 observations is at 500.4 MB, over the <500 MB bar**, and the typical peak is ~497 with a margin under 1 %. A criterion its own measurement straddles is PARTIAL, not MET (`AGENTS.md` §2). The test therefore stops adjudicating this bar and ratchets at 520 MB instead — `PEAK_RSS_RATCHET_MB`, 3.9× the upper mode's span above the observed max, watched failing on a +60 MB injection at `assert 560.59375 < 520` (#104, PR #116). **Corrected mid-measurement and left on the record:** the first 24 runs gave 0 at or above 500 and were written up as met; the 25th–29th produced 500.4. 24 samples could not see the tail of a 0.6 %-margin distribution and 29 cannot either — which is the argument for a ratchet rather than more sampling. The **746 MB** this row previously carried does not reproduce; ~250 MB came off it and nothing recorded what (#52) |
...
| `worker/figures.spec` | **PARTIAL** | ResNet ≥5 ✅ (9, all vector); `is_vector` correct ✅. **The ≥80 % captioned clause is MET; the spec is PARTIAL for a different reason.** The 68.2 % (58/85) this row carried is a **composite over parser regions**, not a caption rate: `_merge_panels` made a no-op takes it to 58/171 — *identical numerator*, 34-point fall — so extent behaviour alone moves it (#111, PR #113). Decomposed against **gold** denominators: caption **detection 25/39 = 64.1 %**; caption **linking given both ends detected 14/15 = 93.3 % ✅**, `n = 15 of 39` (the other 24 have an end never detected); float detection **29/80 type-blind, 22/80 type-aware**. #51 got 7/8 for the linking half by hand; this is that as an assertion over all six papers. The gpt3 type-aware gap stays a convention disagreement — 9 of 10 boxed correctly, typed `table` where gold says `figure`, and they *are* booktabs tables in the GPT-3 source. **What is short is detection, both sides**: neural-odes 1/22 and resnet 5/29 (#103, whose "blocks gate item 5" claim is retracted there). `n = 36 pp / 6 of 8 papers / 442 regions / 1 annotator / no IAA` |

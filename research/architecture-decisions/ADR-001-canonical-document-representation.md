...
3. **Applied literally, it eliminates everything — 0 of 192 pass.** Stated up front rather
   than discovered late. The sole cause is one class of input and it is not an ID problem:
   `bert-2col` p14 draws the figure label *"Tok 1"* **twice, 0.4 pt apart** (a fake-bold /
   shadowed-label render). The two records have different float bboxes so the harness calls
   them distinct, but they are the same ink — a highlight cannot land on the "wrong" one.
...
changes *which* combinations are viable, not *whether any are*. (b) Once R1 has eliminated,
the survivors are ranked by **churn under realistic perturbations** — that is the criterion
the mandated rule lacked entirely, and it is the one that distinguishes 1.0 pt from 0.25 pt.
(c) R3 is the mandated rule, applied last, as a tie-break within one standard error.
...
6. **R1 required an explicit relaxation.** As literally stated it eliminates all 192
   combinations because one figure label in `bert-2col` is drawn twice 0.4 pt apart. R1 is
   evaluated on genuine collisions, excluding duplicate renders (§ C). The exemption is
   grid-independent and both counts are in every row of the raw data, but it **is** a
   relaxation and a reviewer should check it rather than take it: 56 of 192 pass even the
   strict reading.
